...
export default function UserPage() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = vehicles?.map((n) => n.name);
  ...
  console.log('vehicles', dataFiltered);

  const notFound = dataFiltered.length === 0;

  const fetchData = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_APP_BACKEND}/vehicle/vehicle-rentals`);
  ...
  const handleAdd = async (event, name) => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_APP_BACKEND}/vehicle/vehicle-rentals`,
        data
      );
  ...
  const handleUpdate = async (data) => {
    try {
      ...
      const response = await axios.put(
        `${import.meta.env.VITE_APP_BACKEND}/vehicle/vehicle-rentals/${formData._id}`,
        updatedata
      );
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_APP_BACKEND}/vehicle/vehicle-rentals/${id}`
      );
                    ...
                    {column.id === 'category' && (
                      <>
                        ...
                        <Select
                          ...
                          onChange={(e) => handleInputChange(column.id, e.target.value)}
                        >
                          <MenuItem value="TwoWheeler">Two Wheeler (Bike)</MenuItem>
                          <MenuItem value="FourWheeler">Four Wheeler(Cars)</MenuItem>
                        </Select>
                      </>
                    )}
                    {column.id === 'image' && (
                  ...
                  .map((row) => (
                    <>
                      <TableRow hover tabIndex={-1} role="checkbox" selected={selected}>
                        ...
                        <TableCell>{row.contactNumber?.slice(0, 20)}...</TableCell>
