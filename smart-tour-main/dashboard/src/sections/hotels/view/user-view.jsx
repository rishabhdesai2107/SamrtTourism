...
export default function UserPage() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = hotels?.map((n) => n.name);
  ...
  console.log('hotels', dataFiltered);

  const notFound = dataFiltered.length === 0;

  const fetchData = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_APP_BACKEND}/lodgepg/lodgepg`);
  ...
  const handleAdd = async (event, name) => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_APP_BACKEND}/lodgepg/lodgepg`,
        data
      );
      alert('Hotel/Pg Added Succesfully');
      await fetchData();
  ...
  const handleUpdate = async (data) => {
    try {
      ...
      const response = await axios.put(
        `${import.meta.env.VITE_APP_BACKEND}/lodgepg/lodgepg/${formData._id}`,
        updatedata
      );
      alert('Hotel/Pg Updated Succesfully');
      await fetchData();
      setFormData([]);
      handleClose();
      setOpen(null);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_APP_BACKEND}/lodgepg/lodgepg/${id}`
      );
      alert('Hotel/Pg Deleted Succesfully');
      await fetchData();
      setFormData([]);
      handleClose();
      setOpen(null);
