...
export default function UserPage() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = contactus?.map((n) => n.name);
  ...
  console.log('contactus', dataFiltered);

  const notFound = dataFiltered.length === 0;

  const fetchData = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_APP_BACKEND}/contactus/contactus`);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_APP_BACKEND}/contactus/contactus/${id}`
      );
      alert('ContactUs Deleted Succesfully');
      await fetchData();
      setFormData([]);
      handleClose();
      setOpen(null);
  ...
  const columns = [
    ...
    { id: 'ownerFullName', label: 'Owner Name' },
          ...
          <TableContainer sx={{ overflow: 'unset' }}>
            <Table sx={{ minWidth: 800 }}>
              ...
              <TableBody>
                ...
                <TableEmptyRows
                  height={77}
                  emptyRows={emptyRows(page, rowsPerPage, contactus.length)}
                />
