...
export default function UserPage() {
  ...
  console.log('businesses', dataFiltered);

  const notFound = dataFiltered.length === 0;

  const fetchData = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_APP_BACKEND}/business/business`);
  ...
  const handleAdd = async (event, name) => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_APP_BACKEND}/business/business`,
        data
      );
  ...
  const handleUpdate = async (data) => {
    try {
      ...
      const response = await axios.put(
        `${import.meta.env.VITE_APP_BACKEND}/business/business/${formData._id}`,
        updatedata
      );
      alert('Business Updated Succesfully');
      await fetchData();
      setFormData([]);
      handleClose();
      setOpen(null);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_APP_BACKEND}/business/business/${id}`
      );
      alert('Business Deleted Succesfully');
      await fetchData();
      setFormData([]);
      handleClose();
      setOpen(null);
  ...
  const columns = [
    ...
    { id: 'Taluka', label: 'Taluka', align: 'center' },
          ...
          <TableContainer sx={{ overflow: 'unset' }}>
            <Table sx={{ minWidth: 800 }}>
              ...
              <TableBody>
                ...
                <TableEmptyRows
                  height={77}
                  emptyRows={emptyRows(page, rowsPerPage, businesses.length)}
                />
