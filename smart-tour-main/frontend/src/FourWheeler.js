...
export default function FourWheeler() {
  AOS.init();
  const [FourWheelersData, setFourWheelersData] = useState([]);
  ...
  const fetchFourWheelers = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND}/vehicle/vehicle-rentals`
      );
      const FourWheelerData = response.data.data.filter(
        (entry) => entry.category === "FourWheeler"
      );
      setFourWheelersData(FourWheelerData);
  ...
  return (
    <>
      <div className="fourwheeler lppage div-center">
        {FourWheelersData.map((item, index) => (
          <div
            class="cardss"
            key={index}
            data-aos="fade-in"
